...
send({
	...
	params: {
		name: 'extract_paths',
		arguments: {
			content: '{"main": "./src/index.ts", "types": "dist/index.d.ts"}',
			format: 'json',
		},
	},
});
...
setTimeout(() => {
	...
	if (init?.serverInfo?.name !== 'paths-le') {
	...
	if (call?.isError !== false) {
		fail('extract_paths reported an error', JSON.stringify(call));
	}
	const found = call?.structuredContent?.data?.paths?.map((item) => item.value) ?? [];
	// Asserting the actual values, not just "it responded" — a server that
	// answers with an empty list would otherwise pass.
	for (const expected of ['./src/index.ts', 'dist/index.d.ts']) {
		if (!found.includes(expected)) {
			fail(`extract_paths did not find ${expected}`, `got: ${found.join(', ')}`);
		}
	}
