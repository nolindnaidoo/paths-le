 ...
 * `ok` is not `success`.
 *
 * `extractPaths` reports an unsupported format as an error and everything else
 * as a success, so an empty document arrives as a true empty result rather than
 * a failure. Reporting that as an error would have a model announce a problem
 * that did not happen, so `ok` is driven by whether any diagnostic is actually
 * an error.
 */
...
export function toDiagnostics(result: ExtractionResult): readonly Diagnostic[] {
	return result.errors.map((e) => ({
		// The engine has an `info` level; a diagnostic has two, because the only
		// decision a caller makes from one is whether the result is usable. `info`
		// joins `warning` rather than becoming a third value nothing branches on —
		// and never `error`, which would fail a result that is fine.
