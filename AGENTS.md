...
- **A refused document names the refusal; it never comes back as an empty result.** An extractor that cannot read what it was given returns `{ refusal }` rather than `[]`, and `extractPaths` turns that into a `parsing` error with severity `error` — so the MCP envelope answers `ok: false` and the CLI exits 2. A malformed CSV holding `/etc/passwd` used to report no paths and an empty `diagnostics`, which is indistinguishable from a clean file; that silent miss is the class of bug this tool exists to prevent. The CSV reader is the only one that refuses today, and its messages are byte-identical to the crate's.
- **CSV is read by a reader written here, not by `csv-parse`.** `extraction/formats/csv.ts` is the same reader as `crate/src/extract/csv.rs`, rule for rule, because `csv-parse` throws (so nothing can say *which* malformation happened or where) and walks the whitespace after a closing quote a byte at a time (so a no-break space refused a document nobody had mis-quoted). Change one side and you change both — the corpus holds them equal.
- **`src/extraction/**` backs a second frontend, and `crate/fixtures/` is the contract:** `bun scripts/check-extraction-parity.ts` runs the corpus against *this* code, `cargo test` runs it against the crate's. A behaviour change here is a behaviour change for both — update the corpus and both implementations in the same commit, with a CHANGELOG entry. `ci-crate.yml` watches `src/extraction/**` and `src/mcp/**` for exactly this reason, so a drift cannot land green. **What is held equal is the shared `extract_paths` tool, not the two surfaces** — this one is IDE-first (the active buffer, read by a person), the CLI is terminal-first (a tree walk, `--resolve`, exit codes, JSON Lines), and neither owes the other its shape. See `crate/SPEC.md`.
