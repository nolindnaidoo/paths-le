//! The Zed side of Paths-LE.
//!
//! This crate holds no extraction logic. It resolves `paths-le-mcp` from npm and
//! hands Zed a command that launches it over stdio — the same server the VSIX
//! bundles and the same one `npx paths-le-mcp` runs. Keeping it a launcher is
...
const PACKAGE_NAME: &str = "paths-le-mcp";
const SERVER_PATH: &str = "node_modules/paths-le-mcp/server.js";

struct PathsLEExtension;
...
impl zed::Extension for PathsLEExtension {
    fn new() -> Self {
        Self
    }
...
zed::register_extension!(PathsLEExtension);
