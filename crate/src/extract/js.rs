...
/// `value.split(/\s+/)[0]` — the first run of non-whitespace.
pub(crate) fn first_token(value: &str) -> &str {
    value.split(is_js_whitespace).next().unwrap_or("")
}
...
mod tests {
    ...
    #[test]
    fn the_first_token_stops_at_whitespace() {
        assert_eq!(first_token("image.png 2x"), "image.png");
        assert_eq!(first_token("solo"), "solo");
